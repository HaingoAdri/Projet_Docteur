...
public class Personne {
    ...
    public static Personne getAllPersonnes(Connection connection, Date date, String name) throws SQLException {
        Personne personnes = new Personne();
        String sql = "select * from personne where date_consultation = '"+date+"' and nom='"+name+"'";
    ...
    public static List<Personne> getAllPersonnes(Connection connection) throws SQLException {
        ...
        ResultSet resultSet = statement.executeQuery(sql);
            ...
            while (resultSet.next()) {
                int id = resultSet.getInt("id");
                String nom = resultSet.getString("nom");
                Date d = resultSet.getDate("date_consultation");
                Date c = resultSet.getDate("naissance");
                Personne personne = new Personne(id, nom,c,d);
                personnes.add(personne);
            }
        return personnes;
    }

    public static void addPersonne(Connection connection, String nom, Date nais, Date cons, double budget, double temperature) throws SQLException {
        PreparedStatement statement = connection.prepareStatement("INSERT INTO personne (nom,naissance,date_consultation,budget,temperature) VALUES (?,?,?,?,?)");
